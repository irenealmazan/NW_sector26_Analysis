...
#scans = numpy.arange(1,916,1)

scans = [75,76]

for jj in range(len(scans)):
    path = '/home/beams25/USER26ID/savedata/2018R1/20180306/Images/'+str(scans[jj])+'/'
    l_file = os.listdir(path)
    if (os.path.isdir('/home/beams25/USER26ID/savedata/2018R1/20180306/Images/'+str(scans[jj])+'h/')):
        print('directory exists')
    else:  
        os.mkdir('/home/beams25/USER26ID/savedata/2018R1/20180306/Images/'+str(scans[jj])+'h/')
        for ii in range(len(l_file)):
            raster = Image.open(path+l_file[ii])
            imarray=np.array(raster)
            data_h5 = h5py.File('/home/beams25/USER26ID/savedata/2018R1/20180306/Images/'+str(scans[jj])+'h/'+os.path.splitext(l_file[ii])[0]+'.h5','w')
