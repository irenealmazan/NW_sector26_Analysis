function dataout = qbin_mpx3(imname, Tthdet, Rdet, Gamdet, Xdet,Ydet,Samth, Samchi, Samphi, plotflag)
...
Powdflag = 0; % Calculate and display image with powder lines (DISABLED)
...
lat = [4.0788 4.0788 4.0788]; % Au cubic lattice vectors a b c (A)
refl = [0 0 2;0 0 4; 0 2 2; 1 1 1; 1 1 3; 1 3 3; 3 3 3; 0 4 4]; % Si powder reflections to plot
...
if(Powdflag)
    ...
    for i=1:numref
        q1=2*pi*norm((refl(i,:)./lat));
        tempm=(abs(qmat(:,:,4)-q1))<0.01;
        tempm1=tempm1+tempm;    
        [r,c,v] = find(tempm);
        if(min(size(r))>0)
            iref(i)=r(1);jref(i)=c(1);
        else
            iref(i)=-1;jref(i)=-1;
        end
    end
    ...
    if (plotflag==1)
 figure(141);set(141,'Position',[3*imd imb imd imd]);clf;imagesc(impow);axis image;caxis([0 imax/200]);
 title('Image with powder lines');
    for i=1:numref
        if(iref(i)>=0)
text(jref(i),iref(i),['[' num2str(refl(i,1)) num2str(refl(i,2)) num2str(refl(i,3)) ']'],'FontSize',14,'Color',[1,1,1]); 
        end
    end
    pause(0.5);
    end
...
tempm2 = (tempm1-(imdet>500)).^2;
%dataout.outlist = outlist;
dataout.qlist = qlist;
%dataout = sum(sum(tempm2(:,25:450)));
dataout.error=sum(sum(tempm2(:,60:390)));
